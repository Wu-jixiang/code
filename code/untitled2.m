...
% ======================
% 1. Berea sandstone parameters (from Gurevich et al., 2010)
% ======================
...
for fluid_idx = 1:length(fluid_cases)
    ...
    for p_idx = 1:length(P_plot_MPa)
        ...
        for w_idx = 1:length(omega)
            [K_sat_complex, ~] = squirtFlowModel_Gurevich(...
                ...
                K_g, K_dry_P, mu_dry_P, phi_c_P, phi_total, alpha, theta_c, K_h, chi);
    ...
    % Draw asymptotes only for 40 MPa, with exact intersections for water
    p_idx_40 = idx_40MPa;
    P_MPa = P_plot_MPa(p_idx_40);
    [K_dry_P, ~, phi_c_P] = getRockPropertiesAtPressure(...
        P_MPa, P_data_MPa, K_dry_data, mu_dry_data, K_h, phi_c0, P_h);
    
    ft = computeCharacteristicFrequency(fluid, alpha, K_h, K_dry_P, theta_c);
    
    % Determine if mid-frequency asymptote should be drawn 
    mid_factor = 8 * phi_c_P / ( (1/K_dry_P - 1/K_h) * fluid.Kf );
    draw_mid = (strcmp(fluid.name, 'Water') && mid_factor < 0.1);
    
    if strcmp(fluid.name, 'Water')
        % --- Water-saturated: exact intersections between low & mid, mid & high ---
        eta = fluid.eta; Kf = fluid.Kf;
        A = (3/8) * (1/K_dry_P - 1/K_h)^2 * eta * K_dry_P / (alpha^2 * phi_c_P);
        B = (8 * phi_c_P * alpha^2 * K_h) / (3 * eta);
        C = (alpha * phi_c_P * K_h) / sqrt(1.5 * eta * Kf);
        
        if draw_mid
            f_low_mid = sqrt(B/A) / (2*pi);
            
            f_mid_high = (B/C)^2 / (2*pi);
            
            
            f_low_mid = max(1, min(f_low_mid, 1e10));
            f_mid_high = max(1, min(f_mid_high, 1e10));
            
            % Low-frequency asymptote 
            if f_low_mid > 1
                f_low = logspace(0, log10(f_low_mid), 200);
                Q_low = computeLowFreqAsymptote(f_low, fluid, K_dry_P, K_h, phi_c_P, alpha);
                semilogx(f_low, Q_low, 'k--', 'LineWidth', 1.5, ...
                    'DisplayName', 'Low-freq asymptote (\propto \omega)');
            end
            
            % Mid-frequency asymptote 
            if f_mid_high > f_low_mid
                f_mid = logspace(log10(f_low_mid), log10(f_mid_high), 200);
                Q_mid = computeMidFreqAsymptote(f_mid, fluid, K_h, phi_c_P, alpha);
                semilogx(f_mid, Q_mid, 'k-.', 'LineWidth', 1.5, ...
                    'DisplayName', 'Mid-freq asymptote (\propto \omega^{-1})');
            end
            
            % High-frequency asymptote 
            if f_mid_high < 1e10
                f_high = logspace(log10(f_mid_high), 10, 200);
                Q_high = computeHighFreqAsymptote(f_high, fluid, K_h, phi_c_P, alpha);
                semilogx(f_high, Q_high, 'k:', 'LineWidth', 1.5, ...
                    'DisplayName', 'High-freq asymptote (\propto \omega^{-1/2})');
            end
        else
           
            low_upper_factor  = 0.5;
            high_lower_factor = 2;
            f_low_end = min(ft * low_upper_factor, 1e10);
            if f_low_end > 1
                f_low = logspace(0, log10(f_low_end), 200);
                Q_low = computeLowFreqAsymptote(f_low, fluid, K_dry_P, K_h, phi_c_P, alpha);
                semilogx(f_low, Q_low, 'k--', 'LineWidth', 1.5, ...
                    'DisplayName', 'Low-freq asymptote (\propto \omega)');
            end
            f_high_start = max(ft * high_lower_factor, 1);
            if f_high_start < 1e10
                f_high = logspace(log10(f_high_start), 10, 200);
                Q_high = computeHighFreqAsymptote(f_high, fluid, K_h, phi_c_P, alpha);
                semilogx(f_high, Q_high, 'k:', 'LineWidth', 1.5, ...
                    'DisplayName', 'High-freq asymptote (\propto \omega^{-1/2})');
            end
...
function [K_sat, mu_sat] = squirtFlowModel_Gurevich(omega, Kf, eta, K_g, K_dry, mu_dry, phi_c, phi_total, alpha, theta_c, K_h, chi)
    ...
    % Modified frame bulk modulus K_mf (Eq.19)
    term_A = 1 / K_h;
    ...
    % Modified frame shear modulus mu_mf (Eq.20)
    mu_mf = 1 / (1/mu_dry - (4/15) * (1/K_dry - 1/K_mf));
    
    % Gassmann fluid substitution (total porosity)
    inv_K_sat = 1/K_g + phi_total*(1/Kf - 1/K_g) / ...
        (1/Kf + phi_total*(1/Kf - 1/K_g) / (1/K_mf - 1/K_g));
    K_sat = 1 / inv_K_sat;
    mu_sat = mu_mf;
end
...
function [K_h, phi_c0, P_h, alpha, theta_c] = estimateShapiroParams(P_MPa, K_dry_data, K_g, mu_g)
    ...
    try
        ...
        fprintf('Parameter estimation results (nonlinear fitting):\n');
    catch
        warning('Optimization Toolbox not found, using linear fitting; results may be less accurate.');
        K_h = K_dry_data(end);
        inv_K_h = 1 / K_h;
        ...
        fprintf('Parameter estimation results (linear fitting):\n');
    end
    theta_c = K_h / P_h;
    alpha = 0.01;
    fprintf('  K_h = %.2f GPa\n', K_h/1e9);
...
function Q_low = computeLowFreqAsymptote(f, fluid, K_dry, K_h, phi_c, alpha)
    omega = 2 * pi * f;
