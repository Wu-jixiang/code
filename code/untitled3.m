%% Heterogeneous fluid squirt-flow model: homogeneous vs heterogeneous comparison at 20 MPa
clear; close all; clc;
...
% Estimate Shapiro model parameters (K_h, phi_c0, P_h, alpha, theta_c)
...
chi_values = [0, 0.3 + 0.15i];
chi_names = {'Homogeneous model', 'Heterogeneous model'};
P_target_MPa = 20;   
freq = logspace(0, 10, 1000); 
omega = 2 * pi * freq;

% Get dry moduli and soft porosity at 20 MPa
[K_dry_20, mu_dry_20, phi_c_20] = getRockPropertiesAtPressure(...
    P_target_MPa, P_data_MPa, K_dry_data, mu_dry_data, K_h, phi_c0, P_h);
...
for fluid_idx = 1:length(fluid_cases)
    ...
    for chi_idx = 1:length(chi_values)
        ...
        for w_idx = 1:length(omega)
            [K_sat_complex, ~] = squirtFlowModel_Gurevich(...
                omega(w_idx), fluid.Kf, fluid.eta, K_g, ...
                K_dry_20, mu_dry_20, phi_c_20, alpha, theta_c, K_h, chi);
            K_sat_real(w_idx, chi_idx) = real(K_sat_complex);
            Qinv(w_idx, chi_idx) = imag(K_sat_complex) / real(K_sat_complex);
        end
    end
    
    
    figure('Position', [100, 100, 1200, 500], ...
        'Name', sprintf('%s-saturated (20 MPa) homogeneous vs heterogeneous', fluid.name));
    
    % Subplot 1: Real part of bulk modulus (dispersion)
    subplot(1,2,1); hold on; grid on; box on;
    plot(freq, K_sat_real(:,1)/1e9, 'k-', 'LineWidth', 2, 'DisplayName', chi_names{1});
    plot(freq, K_sat_real(:,2)/1e9, 'k--', 'LineWidth', 2, 'DisplayName', chi_names{2});
    set(gca, 'XScale', 'log');
    xlabel('Frequency (Hz)'); ylabel('Bulk modulus {\it K}_{\rm sat} (GPa)');
    title(sprintf('%s-saturated (20 MPa): bulk modulus dispersion', fluid.name));
    legend('Location','best');
    xlim([1e0, 1e10]);
    
    % Subplot 2: Inverse quality factor (attenuation)
    subplot(1,2,2); hold on; grid on; box on;
    plot(freq, Qinv(:,1), 'k-', 'LineWidth', 2, 'DisplayName', chi_names{1});
    plot(freq, Qinv(:,2), 'k--', 'LineWidth', 2, 'DisplayName', chi_names{2});
    set(gca, 'XScale', 'log', 'YScale', 'log');
    xlabel('Frequency (Hz)'); ylabel('{\it Q}^{-1}');
    title(sprintf('%s-saturated (20 MPa): attenuation', fluid.name));
    xlim([1e0, 1e10]);
    ...
    sgtitle(sprintf('Berea sandstone 20 MPa, %s-saturated', fluid.name), 'FontWeight','bold');
end

%% ====================== Function definitions ======================
function [K_sat, mu_sat] = squirtFlowModel_Gurevich(omega, Kf, eta, K_g, K_dry, mu_dry, phi_c, alpha, theta_c, K_h, chi)
% Modified squirt-flow model (including heterogeneity coefficient chi)
% Inputs:
%   omega : angular frequency
%   Kf    : fluid bulk modulus
%   eta   : fluid viscosity
%   K_g   : grain bulk modulus
%   K_dry : dry rock bulk modulus (at given pressure)
%   mu_dry: dry rock shear modulus
%   phi_c : soft porosity (at given pressure)
%   alpha : soft pore aspect ratio
%   theta_c: stress sensitivity parameter (from Shapiro model)
%   K_h   : high-pressure limit dry bulk modulus
%   chi   : heterogeneity coupling coefficient (complex, 0 = homogeneous)
% Outputs:
%   K_sat : complex saturated bulk modulus
%   mu_sat: complex saturated shear modulus
...
% Estimate parameters according to Shapiro (2003) model
    P_Pa = P_MPa * 1e6;
...
% Compute dry rock moduli from velocity data
    Vp = Vp(:); Vs = Vs(:);
    mu_dry = rho * Vs.^2;
